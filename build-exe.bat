...
if errorlevel 1 (
  ...
  if errorlevel 1 (
    ...
    echo Close this window, open a new terminal, and run build-exe.bat again.
    pause
    exit /b 1
  )
...
if not exist "tools\yt-dlp.exe" (
  ...
  echo yt-dlp downloaded - it will be bundled straight into the .exe.
) else (
  echo yt-dlp found in tools\ - it will be bundled into the .exe.
)

echo.

set BUILD_SCRIPT=dist:win:fast
set MODE_LABEL=fast test build ^(uncompressed exe, builds quicker^)
...
for %%A in (%*) do (
  ...
  if /i "%%A"=="--release" (
    set BUILD_SCRIPT=dist:win
    set MODE_LABEL=release build ^(compressed, smaller exe, slower^)
  )
)
...
if not exist "node_modules" (
  ...
  echo       ^(run "build-exe.bat --clean" if something's acting weird^)
)

echo.
echo [2/2] Building the Windows .exe...
